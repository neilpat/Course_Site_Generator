...
function buildTACell(counter, numTAs, ta) {
    ...
    var subabbrName = subname.replace(/\s/g, '');
    var email = ta.email;
    var text = "<td class='tas'><img width='100' height='100'"
                + " src='./images/tas/" + abbrName + ".JPG' "
                + " onerror= this.src='./images/tas/" + subabbrName + ".JPG'"
                + " alt='" + name + "' /><br />"
...
function loadAppDetails(){
    ...
    loadData(dataFile, replaceTitle);
    loadData(dataFile, replaceImage);
    loadData(dataFile, addLink);
}
